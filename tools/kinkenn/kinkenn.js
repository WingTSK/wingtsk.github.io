 ...
 * 
 * kinkenn.js */ version = '0.0.7.2 Beta'; /* 2024-06-28
 *
 * Based on draw-calculator.js ver.0.3.2.2
 */
...
/*kinkenn*/
function kinkennResult(deck, hand, cnums, condition, pt){
  let other = deck - sumArray(cnums);
  let coordinate = new Array(cnums.length).fill(0);
  ...
  return patKinkennResult(deck, hand, cnums, condition, other, coordinate, coorlen, cv, depth, pattern, pt, kinkennmode);
}

/*kinkenn*/
function patKinkennResult(deck, hand, cnums, condition, other, coordinate, coorlen, cv, depth, pattern, pt, kinkennmode){
  let rtn = 0;
  let cd = cnums[depth];
  for (coordinate[depth] = 0; cv + coordinate[depth] <= hand; coordinate[depth] = (coordinate[depth] + 1)){
    if (coordinate[depth] <= cd){
      ...
      if (depth < coorlen){
        rtn = rtn + patKinkennResult(deck, hand, cnums, condition, other, coordinate, coorlen, cv + coordinate[depth], depth + 1, nextpattern, pt, kinkennmode);
      }else{
        if (other >= hand - (cv + coordinate[depth])){
          if (chkPattern(coordinate, condition)){
            rtn = rtn + (nextpattern * combination_pt(other, hand - (cv + coordinate[depth]), pt));
          }else{
            let k = chkKinkenn(deck, hand, cnums, coordinate, condition, kinkennmode, pt);
            /*console.log(k, coordinate);*/
            rtn = rtn + (nextpattern * combination_pt(other, hand - (cv + coordinate[depth]), pt) * k);
          }
        }
      }
    }
...
function chkKinkenn(deck, hand, cnums, coordinate, condition, kinkennmode, pt){
  if (coordinate[0] > 0){
    ...
    for (let i = 0; i < len; i += 1){
      a = [];
      tc = 1;
      for (let j = 1, jl = condition[i].length; j < jl && tc === 1; j += 1){
    ...
    return 1 - combination_pt(deck - hand - s, kinkennmode, pt) / combination_pt(deck - hand, kinkennmode, pt);
  }else{
    return 0;
  }
}
...
function makeConditionBlock(group, conCoor, mode){
  ...
  return new Array(cclen).fill().map(function(_, i){
    return new Array(gl).fill().map(function(_, j){
...
function makeConditionGroup(ary1, ary2){
  if (ary1.length + ary2.length){
    let rtn = [];
    let alen = ary1.length;
    let blen = ary2.length;
...
function getCardsNums(){
  ...
  return new Array(cards.length).fill().map(function(_, i){
    return Number(cards[i].querySelector('.cardnum').value);
  });
}
    ...
    if (deck >= kinkenn + sumArray(cnums)){
      ...
      let _cards = (function(consource){
        let ary = [];
          ...
          return [0].concat(ary.uniq().sort(function(a, b){return a - b;}));
        })(consource);
      ...
      if (condition.length > 0){
        ...
        let result = kinkennResult(deck, hand, _cnums, condition, pt);
        outputTop(makeTopOutputMsgK(result, deck, hand, pt, kinkenn));
        ...
        for (let m = 0; m < cl; m += 1){
          ...
          if (mscon.length > 0){
            ...
            if (JSON.stringify(condition) === JSON.stringify(mscon)){
              ...
              msresult = kinkennResult(deck, hand, _cnums, mscon, pt);
            }
            outputCon('個別' + makeTopOutputMsgK(msresult, deck, hand, pt, kinkenn), m);
          }else{
